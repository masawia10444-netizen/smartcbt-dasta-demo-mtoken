...
const carbonProjectStatusBadgeCell = cva("h-8 w-fit rounded-2xl px-4 py-1 text-base", {
  variants: {
    status: {
      ...
      request_to_delete: "bg-[#FFF7E6] text-smart-cbt-orange",
      deleted: "bg-[#FFE3E1] text-smart-cbt-red",
    },
  },
});

interface CarbonProjectStatusBadgeCellProps extends HtmlHTMLAttributes<HTMLDivElement> {
  status: PROJECT_STATUS;
}

export const CarbonProjectStatusBadgeCell = (props: CarbonProjectStatusBadgeCellProps) => {
  const { status } = props;
  const t = useTranslations("common");

  return (
    <div className={cn(carbonProjectStatusBadgeCell({ status: status as unknown as PROJECT_STATUS }))}>
