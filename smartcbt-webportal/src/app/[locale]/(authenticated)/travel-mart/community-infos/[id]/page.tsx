...
import { fetchCommunityFormById } from "@/utils/cms/adapters/website/travel-mart/communities/communities";
...
export default async function CommunityInfoFormPage({ params }: { params: { id: number } }) {
  ...
  console.time("first");
  const [communityForm, attractionTypes, touristTargetGroups, months, facilities] = await Promise.all([
    fetchCommunityFormById(profile, params.id),
    fetchAttractionType(),
    fetchTouristTargetGroups(),
    listMonths(),
    fetchFacilities(),
  ...
  console.time("second");
  const [provinces, districts, subdistricts, regions, countries] = await Promise.all([
    fetchProvinces(),
    fetchDistricts(),
    fetchSubDistricts(),
    fetchRegions(),
  ...
  return (
    <AppContextProvider provinces={provinces} districts={districts} subdistricts={subdistricts}>
      <TravelMartContextProvider attractionTypes={attractionTypes}>
        <CommunityInfoPortalForm communityForm={communityForm} months={months} step2={step2} step3={step3} />
